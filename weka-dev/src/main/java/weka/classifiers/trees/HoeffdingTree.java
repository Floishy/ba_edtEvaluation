...
public class HoeffdingTree extends AbstractClassifier implements
	...
	private LinkedDataExpansionTool expansionTool;

	private TestConfiguration testConfiguration;

	private ISplitExpand splitExpandAlgorithm;

	protected Instances m_header;
	protected HNode m_root;

	/** The number of instances a leaf should observe between split attempts */
	protected double m_gracePeriod = 1;
	...
	public void buildClassifier(Instances data) throws Exception {
		...
		for (int i = 0; i < data.numInstances(); i++) {
			...
			instance = expandInstance(data.instance(i));
			// data.set(i,));
	...
	private Instance expandInstance(Instance instance) {

		instance = expansionTool.expand(instance);

		return instance;
	}

	public ISplitExpand getSplitExpandAlgorithm() {
		return splitExpandAlgorithm;
	}

	public void setSplitExpandAlgorithm(ISplitExpand splitExpandAlgorithm) {
		this.splitExpandAlgorithm = splitExpandAlgorithm;
		// this.splitExpandAlgorithm.setHoeffdingTree(this);
	...
	public void updateClassifier(Instance inst) throws Exception {
		...
		// XXX YTE Initially leaning node, but later split node which is used to
		...
		if (actualNode instanceof LearningNode) {
			...
			if (/* m_growthAllowed && */actualNode instanceof ActiveHNode) {
				// XXX YTE The total number of examples seen so far
				double totalWeight = actualNode.totalWeight();
				// System.out.println(totalWeight+" "+((ActiveHNode)
				// actualNode).m_weightSeenAtLastSplitEval);
				// XXX YTE check if enough examples have been seen so far to
				// check again for a possible split
				if (totalWeight
						- ((ActiveHNode) actualNode).m_weightSeenAtLastSplitEval > m_gracePeriod) {
					// try a split
					// trySplit((ActiveHNode) actualNode, l.m_parentNode,
					// l.m_parentBranch);
					this.getSplitExpandAlgorithm().trySplit(
							(ActiveHNode) actualNode, l.m_parentNode,
							l.m_parentBranch);
			...
			throw new RuntimeException(
					"Instance should always land in a learning node");
		}
	}
	...
	protected void replaceNode(ActiveHNode node, ActiveHNode replacementNode,
			SplitNode parent, String parentBranch) {

		if (parent == null) {
			m_root = replacementNode;
		} else {
			parent.setChild(parentBranch, replacementNode);
		}
	...
	public void updateHeaderAttributes() {
		Attributes currentSchema = this.expansionTool.getCurrentSchema();
		//System.out.println(currentSchema.toString());
		m_header = new Instances("a", currentSchema, 0);
	...
	public LinkedDataExpansionTool getExpansionTool() {
		return this.expansionTool;
	}

	public void setExpansionTool(LinkedDataExpansionTool expansionTool) {
	...
	public void setM_gracePeriod(double m_gracePeriod) {
		this.m_gracePeriod = m_gracePeriod;
	}

	public void setM_splitConfidence(double m_splitConfidence) {
		this.m_splitConfidence = m_splitConfidence;
	}

	public void setM_hoeffdingTieThreshold(double m_hoeffdingTieThreshold) {
		this.m_hoeffdingTieThreshold = m_hoeffdingTieThreshold;
	}

	public void setM_minFracWeightForTwoBranchesGain(
			double m_minFracWeightForTwoBranchesGain) {
		this.m_minFracWeightForTwoBranchesGain = m_minFracWeightForTwoBranchesGain;
	}
	...
	public void setOptions(String[] options) throws Exception {
		...
		if (opt.length() > 0) {
			setSplitCriterion(new SelectedTag(Integer.parseInt(opt),
					TAGS_SELECTION));
		}
