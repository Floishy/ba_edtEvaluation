...
RevisionHandler {
  ...
  public void insertAttributeAt(/* @non_null@ */Attribute att, int position) {
    ...
    for (int i = 0; i < numInstances(); i++) {
      //TODO YTE Not all instances have the same length
      instance(i).setDataset(null);
  ...
  public ArrayList<Attribute> getM_Attributes() {
	return m_Attributes;
}
  ...
  /**
   * XXX YTE
   * Returns true if the instances have attibute named like that
   * 
   * @param attributeName
   * @return
   */
  ...
  public String getAttributesSchema(){
	  StringBuffer text = new StringBuffer();
	  for(int i=0;i<m_Attributes.size();i++){
		  text.append(m_Attributes.get(i).m_Name);
		  text.append(";");
	  }
	  return text.toString();
  }
